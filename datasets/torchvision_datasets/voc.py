...
class VOCDetection(VisionDataset):
    ...
    def __init__(
        ...
        for year, image_set in zip(years, image_sets):
            ...
            self.images.extend(
                [os.path.join(image_dir, x + ".jpg") for x in file_names]
            )
    ...
    def __getitem__(self, index):
        ...
        target = dict(
            labels=torch.tensor(
                [i["category_id"] for i in instances], dtype=torch.int64
            ),
            image_id=torch.tensor(
                [int(image_id.replace("_", "").replace(".jpg", ""))], dtype=torch.int64
            ),
