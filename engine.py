    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if "savedet" in output_dir:
            ...
            for i, target in enumerate(targets):
                ...
                pred_boxes_ = box_cxcywh_to_xyxy(pred_boxes) * torch.stack(
                ...
                torch.save(
                    dict(
                        ...
                        pred_boxes_=pred_boxes_,
                    ),
                    os.path.join(output_dir, str(image_id) + ".pt"),
                )
...
def viz(model, criterion, postprocessors, data_loader, base_ds, device, output_dir):
    ...
    for samples, targets in data_loader:
        ...
        predictied_boxes = torch.stack(
            [outputs["pred_boxes"][0][i] for i in indices]
        ).unsqueeze(0)
        logits = torch.stack([outputs["pred_logits"][0][i] for i in indices]).unsqueeze(
            0
        )
        ...
        plot_prediction(
            samples.tensors[0:1],
            boxes_ss,
            torch.zeros(1, boxes_ss.shape[1], 4).to(logits),
            ax[0],
            plot_prob=False,
        )
        ...
        # Pred results
        plot_prediction(
            samples.tensors[0:1], predictied_boxes, logits, ax[1], plot_prob=False
        )
        ...
        plot_prediction(
            ...
            torch.zeros(1, targets[0]["boxes"].shape[0], 4).to(logits),
            ax[2],
            plot_prob=False,
        )
        ax[2].set_title("GT")
        ...
        plt.savefig(
            os.path.join(output_dir, f'img_{int(targets[0]["image_id"][0])}.jpg')
        )
